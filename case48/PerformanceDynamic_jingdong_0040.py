...
class PerformanceDynamic_jingdong_0040(Case):
    ...
    def run_case(self):
        ...
        for test_time in range(0, self.TEST_TIME):
            ...
            # 1、启动京东，等待3s
            logging.info('启动京东')
            SeaOfStarsAW.trace_thread.add_log('京东', '1、启动京东，等待3s')
            # for _ in range(4):
            ...
            SeaOfStarsAW.trace_thread.add_log('京东', '3、浏览直播页面，上滑5次，下滑5次，每次停留2s')
            for i in range(5):
            ...
            # 7、点击右边购物袋，每次停留2s
            SeaOfStarsAW.trace_thread.add_log('京东', '7、点击右边购物袋，每次停留2s')
            SeaOfStarsAW.ut_device.click(0.918, 0.934)
            time.sleep(2)

            # 8、浏览购物袋，上滑5次，下滑5次，每次停留2s
            SeaOfStarsAW.trace_thread.add_log('京东', '8、浏览购物袋，上滑5次，下滑5次，每次停留2s')
            for i in range(5):
                SeaOfStarsAW.ut_device.swipe_up()
                time.sleep(2)
                SeaOfStarsAW.ut_device.swipe_down()
            ...
            # 9、返回主页面，等待2s
            SeaOfStarsAW.trace_thread.add_log('京东', '9、返回主页面，等待2s')
            for i in range(3):
                SeaOfStarsAW.ut_device.swipe(0.01, 0.5, 0.8, 0.5, duration=0.03)
                time.sleep(2)

            # 10、点击我的，等待2s
            SeaOfStarsAW.trace_thread.add_log('京东', '10、点击我的，等待2s')
            SeaOfStarsAW.ut_device(labelContains="我的").click()
            time.sleep(2)
            # SeaOfStarsAW.ut_device.click(0.5, 0.5)

            # 11、点击全部订单，等待2s
            SeaOfStarsAW.trace_thread.add_log('京东', '11、点击全部订单，等待2s')
            SeaOfStarsAW.ut_device(label="全部").click()
            time.sleep(2)

            # 12、浏览全部订单页面，上滑2次，下滑2次，每次停留2s
            SeaOfStarsAW.trace_thread.add_log('京东', '12、浏览全部订单页面，上滑2次，下滑2次，每次停留2s')
            for i in range(2):
                SeaOfStarsAW.ut_device.swipe_up()
                time.sleep(2)
            for i in range(3):
            ...
            # 13、点击待收货，停留2s
            SeaOfStarsAW.trace_thread.add_log('京东', '13、点击待收货，停留2s')
            SeaOfStarsAW.ut_device(labelContains="待收货").click()
            time.sleep(2)

            # 14、返回首页，等待2s
            SeaOfStarsAW.trace_thread.add_log('京东', '14、返回首页，等待2s')
            SeaOfStarsAW.ut_device.swipe(0.01, 0.5, 0.8, 0.5, duration=0.03)
            time.sleep(2)
            ...
            # 15、返回home页面，等待2s
            SeaOfStarsAW.trace_thread.add_log('京东', '15、返回home页面，等待2s')
