...
class PerformanceDynamic_qq_0010(Case):
    ...
    def run_case(self):
        ...
        for test_time in range(0, self.TEST_TIME):
            ...
            time.sleep(2)
            logging.info('点击好友动态')
            SeaOfStarsAW.ut_device(label='空间动态').click()
            time.sleep(3)
            logging.info('点击我的头像进入空间')
            SeaOfStarsAW.ut_device.click(0.133, 0.236)
            time.sleep(2)
            logging.info('点击动态图片')
            SeaOfStarsAW.ut_device.click(0.212, 0.075)
            time.sleep(2)
            logging.info('左滑3次')
            for _ in range(3):
                SeaOfStarsAW.ut_device.swipe_left()
                time.sleep(3)
            logging.info('返回')
            SeaOfStarsAW.ut_device.click(0.075, 0.09)
            time.sleep(2)
            logging.info('返回')
            SeaOfStarsAW.ut_device().click(0.075, 0.09)
            time.sleep(2)
            ...
            time.sleep(2)
            SeaOfStarsAW.ut_device.app_terminate("com.tencent.mqq")
