...
class Performance_jank_kpi_05_000048(Case):
    ...
    def run_case(self):
        ...
        for test_time in range(0, self.TEST_TIME):
            ...
            # 点击视频号
            logging.info('点击视频号')
            SeaOfStarsAW.trace_thread.add_log('微信', '点击视频号')
            SeaOfStarsAW.check_status(xpath='//Table/Cell[2]')
            SeaOfStarsAW.ut_device.xpath('//Table/Cell[2]').click()
            time.sleep(2)
            time.sleep(5)
            step += 1
            SeaOfStarsAW.ut_device.screenshot(self.screenshot_dir_path + '/' + self.__class__.__name__ + 'step_' + str(step) + '_' +time.strftime('%H%M%S', time.localtime()) + '.png')
            ...
            # 下滑一次
            logging.info('下滑一次')
            SeaOfStarsAW.trace_thread.add_log('微信', '下滑一次')
