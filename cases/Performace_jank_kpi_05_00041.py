...
class Performance_jank_kpi_05_000041(Case):
    ...
    def run_case(self):
        ...
        for test_time in range(0, self.TEST_TIME):
            ...
            # 进入收付款
            SeaOfStarsAW.trace_thread.add_log('微信', '进入收付款')
            # SeaOfStarsAW.check_status(xpath='//CollectionView/Other[1]/Other[1]/Other[1]/Other[1]/Image[1]')
            # SeaOfStarsAW.ut_device(name='pay_regular').click()
            SeaOfStarsAW.ut_device.click(0.278, 0.172)
            time.sleep(2)
            step += 1
            # SeaOfStarsAW.ut_device.screenshot(self.screenshot_dir_path + '/' + self.__class__.__name__ + 'step_' + str(step) + '_' +time.strftime('%H%M%S', time.localtime()) + '.png')
