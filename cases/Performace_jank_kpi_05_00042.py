...
class Performance_jank_kpi_05_000042(Case):
    ...
    def run_case(self):
        ...
        for test_time in range(0, self.TEST_TIME):
            ...
            # 点击搜索图标
            logging.info('点击搜索图标')
            SeaOfStarsAW.trace_thread.add_log('微信', '点击搜索图标')
            # SeaOfStarsAW.check_status(xpath='//Table/SearchField[2]')
            time.sleep(2)
            SeaOfStarsAW.ut_device.click(0.501, 0.146)
            time.sleep(2)
            step += 1
            # SeaOfStarsAW.ut_device.screenshot(self.screenshot_dir_path + '/' + self.__class__.__name__ + 'step_' + str(step) + '_' +time.strftime('%H%M%S', time.localtime()) + '.png')
            ...
            # time.sleep(2)
            SeaOfStarsAW.ut_device(label="华为手机").click()
            time.sleep(2)
            step += 1
            # SeaOfStarsAW.ut_device.screenshot(self.screenshot_dir_path + '/' + self.__class__.__name__ + 'step_' + str(step) + '_' +time.strftime('%H%M%S', time.localtime()) + '.png')
