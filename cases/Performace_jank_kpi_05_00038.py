...
class Performance_jank_kpi_05_000038(Case):
    ...
    def run_case(self):
        ...
        for test_time in range(0, self.TEST_TIME):
            ...
            # 进入通讯录
            logging.info('进入通讯录')
            SeaOfStarsAW.trace_thread.add_log('微信', '进入通讯录')
            ...
            # 进入好友"测试"的详情界面
            SeaOfStarsAW.check_status(label='测试')
            for i in range(6):
            ...
            # 点击好友的朋友圈
            SeaOfStarsAW.trace_thread.add_log('微信', '点击好友的朋友圈')
            for _ in range(3):
                SeaOfStarsAW.check_status(label='朋友圈')
                logging.info('点击好友的朋友圈')
            ...
            # 浏览图片
            SeaOfStarsAW.trace_thread.add_log('微信', '浏览图片')
            for j in range(6):
                SeaOfStarsAW.ut_device.click(0.293, 0.572)
                time.sleep(2)
                SeaOfStarsAW.ut_device.swipe(0.9, 0.4, 0.3, 0.4, 0.05)
                time.sleep(2)
                SeaOfStarsAW.ut_device.swipe(0.524, 0.208, 0.524, 0.447, 0.05)
                time.sleep(2)
            step += 1
            # SeaOfStarsAW.ut_device.screenshot(self.screenshot_dir_path + '/' + self.__class__.__name__ + 'step_' + str(step) + '_' +time.strftime('%H%M%S', time.localtime()) + '.png')
            ...
            time.sleep(2)
            SeaOfStarsAW.check_status(label='把他推荐给朋友')
            SeaOfStarsAW.ut_device(label='把他推荐给朋友').click()
            time.sleep(2)
            step += 1
            # SeaOfStarsAW.ut_device.screenshot(self.screenshot_dir_path + '/' + self.__class__.__name__ + 'step_' + str(step) + '_' +time.strftime('%H%M%S', time.localtime()) + '.png')

            # 返回微信主界面
            logging.info('返回微信主界面')
            SeaOfStarsAW.trace_thread.add_log('微信', '返回微信主界面')
            ...
            # 点击微信界面
            logging.info('点击微信界面')
            SeaOfStarsAW.trace_thread.add_log('微信', '点击微信界面')
