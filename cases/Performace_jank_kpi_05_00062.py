...
class Performance_jank_kpi_05_000062(Case):
    ...
    def run_case(self):
        ...
        for test_time in range(0, self.TEST_TIME):
            ...
            # 应用启动
            logging.info('应用启动')
            SeaOfStarsAW.ut_device(label="电话").click()
            time.sleep(2)
    
            # 拨打历史通话记录
            logging.info('拨打历史通话记录')
            SeaOfStarsAW.check_status(label='最近通话')
            SeaOfStarsAW.ut_device(label="最近通话").click()
            time.sleep(2)
            step += 1
